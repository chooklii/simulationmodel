...
from clients.usersession import (
    UserSession,
    getAllSessionsToRequest,
    updateCompletedSession,
    getSessionByID,
)
from clients.request import HttpRequest
from clients.userrequest import (
    UserRequest,
...
from components.handler import (
    allComponents,
    initExternalSystemsMW,
    initExternalSystemsMQ,
)
from components.externalSystems import (
    getExternalSystemsToWrite,
    checkIfRequestIncludedAnyInitiator,
)

from validation import validateConfigData
...
def request_creator(
    ...
    while True:
        ...
        processExternalSystemsRQ(externalSystemsRQ, env, requestOptions)
        if len(requestsToSend) != 0:
            ...
            yield env.timeout(config.TIMEOUT_INTERVALL)


def request_requests(env, req, destination, externalSystemsMW, requestOptions):
    global allSessions, ssl
...
def request_creator_sessions(env, sessions, thinktime, requestOptions,
                             externalSystemsMQ, externalSystemsMW, externalSystemsRQ, ssl):
    global allSessions
...
def processExternalSystemsRQ(externalSystemsRQ, env, requestOptions):
    global externalSystemSessions
